  ...
  return (
    <section className="row">
      ...
      <section className="my-3">
        <main className="container-lg col-12 d-flex flex-column  align-items-start">
          <section className="my-3 my-headings-2 ">
            <h1 className="quick-search-text h3 fw-bold fs-2 text-dark ms-lg-0 ms-2">
              Quick Search's
            </h1>
            <p className="text-muted  ms-lg-0 ms-2">
              Discover restaurants by type of meal
            </p>
          </section>

          <section className=" d-flex justify-content-between flex-wrap ms-lg-0 ms-2 my-4 meal_type_list ">
            {/* artcles list */}
            <div className="w-100 d-flex flex-wrap justify-content-center gap-2  ">
              {mealtypesList.map((type) => (
                <article
                  key={type._id}
                  className="article-shadow  mb-lg-4 mb-2 gap-lg-0 meal-member gap-3 d-flex justify-content-start align-items-center bg-white pe-lg-4 p-0 "
                  onClick={() => {
                    navigate(`meal/${type._id}/${type.name}`);
                  }}
                >
                  <img
                    src={type.image}
